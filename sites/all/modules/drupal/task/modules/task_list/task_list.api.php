 ...
 */
function hook_task_list_load(array $entities) {
 ...
 */
function hook_task_list_insert(Task $task) {
  db_insert('mytable')
    ->fields(array(
 ...
 */
function hook_task_list_presave(Task $task) {
  $task->name = 'foo';
}
 ...
 */
function hook_task_list_update(Task $task) {
  db_update('mytable')
 ...
 */
function hook_task_list_delete(Task $task) {
  db_delete('mytable')
 ...
 */
function hook_task_list_view($task, $view_mode, $langcode) {
 ...
 */
function hook_task_list_view_alter($build) {
 ...
 * @see hook_entity_load()
 */
function hook_task_list_type_load(array $entities) {
  $result = db_query('SELECT pid, foo FROM {mytable} WHERE pid IN(:ids)', array(':ids' => array_keys($entities)));
  foreach ($result as $record) {
    $entities[$record->pid]->foo = $record->foo;
  }
 ...
 * @see hook_entity_insert()
 */
function hook_task_list_type_insert(TaskType $task_type) {
  db_insert('mytable')
    ->fields(array(
 ...
 * @see hook_entity_presave()
 */
function hook_task_list_type_presave(TaskType $task_type) {
  $task_type->name = 'foo';
}
 ...
 * @see hook_entity_update()
 */
function hook_task_list_type_update(TaskType $task_type) {
  db_update('mytable')
 ...
 * @see hook_entity_delete()
 */
function hook_task_list_type_delete(TaskType $task_type) {
  db_delete('mytable')
